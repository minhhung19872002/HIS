...
/* ────────────────────────────────────────────────────────────
   QueueDisplay v2 — TV board màn hình chờ công cộng (#415).
   Port từ v1 pages/QueueDisplay.tsx (logic verbatim): TTS vi-VN +
   beep AudioContext + lab queue 5 trạng thái + KPI + polling 4s.
   ?type=general (mặc định, cần ?rooms=id1,id2) | ?type=lab.
   Kiosk KHÔNG port (v2 đã có KioskSelfService); Zone view giữ ở v1.
   Route public standalone /v2/queue-display (không auth, không shell).
   ──────────────────────────────────────────────────────────── */
...
function LabQueueView() {
  ...
  return (
    <div className="queue-display lab-mode v2">
      {showOverlay && (
...
function RoomQueueView() {
  ...
  return (
    <div className="queue-display v2">
      {showOverlay && (
        <div className="audio-overlay">
          <button onClick={enableAudio}>Bật âm thanh</button>
