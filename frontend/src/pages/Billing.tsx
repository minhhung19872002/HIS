...
import { parseCurrencyInput } from './billing/utils';
import {
  mapDepositDto,
  mapRefundDto,
  mapUnpaidServiceDto,
} from './billing/mappers';
import { numberToWords } from './billing/numberToWords';
import {
  buildReceiptHtml,
  buildDepositHtml,
  buildRefundHtml,
...
const Billing: React.FC = () => {
  ...
  const executePrintReceipt = () => {
    ...
    printWindow.document.write(buildReceiptHtml(selectedPatient, selectedItems, totals));
  ...
  const executePrintDeposit = (deposit: Deposit) => {
    ...
    printWindow.document.write(buildDepositHtml(deposit));
    printWindow.document.close();
    printWindow.focus();
    setTimeout(() => { printWindow.print(); }, 500);
  };
  ...
  const executePrintRefund = (refund: RefundRecord) => {
    ...
    printWindow.document.write(buildRefundHtml(refund));
    printWindow.document.close();
    printWindow.focus();
    setTimeout(() => { printWindow.print(); }, 500);
  };
  ...
  // numberToWords moved to ./billing/numberToWords.ts (K16 B2)
