#!/usr/bin/env python
"""Background retry loop for ARM Ampere A1 capacity.

Attempts to launch his-pacs-vm (ARM 4 OCPU, 24 GB) every 5 minutes.
When capacity frees up, writes vm-info-arm.txt with the new VM details.
...
def try_launch(config, state: dict) -> dict | None:
    ...
    # Skip if instance already exists and is usable
    existing = compute.list_instances(compartment_id=tenancy_id, display_name="his-pacs-vm").data
    active = [i for i in existing if i.lifecycle_state in ("RUNNING", "PROVISIONING", "STARTING")]
    if active:
        log(f"ARM instance already exists: {active[0].id} ({active[0].lifecycle_state})")
        return None
    ...
    log(f"ARM launched: {inst.id} — waiting for RUNNING...")
    ...
    state.update({
        "arm_instance_id": inst.id,
        "arm_public_ip": public_ip,
        "arm_private_ip": private_ip,
    })
...
def main() -> int:
    ...
    attempt = 0
    start = time.time()
    max_duration = 4 * 3600  # bail after 4 hours

    log("Starting ARM capacity retry loop (5-min interval, 4h cap)...")
    while True:
        attempt += 1
        ...
        if result:
            log(f"SUCCESS! ARM VM ready at {result['public_ip']}")
            (HERE / "vm-info-arm.txt").write_text(
