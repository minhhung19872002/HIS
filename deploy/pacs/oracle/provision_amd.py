#!/usr/bin/env python
"""Provision AMD Micro VM as temporary fallback while waiting for ARM capacity.

Uses the VCN/subnet/SSH key created by provision.py.
...
def main() -> int:
    ...
    existing = compute.list_instances(compartment_id=tenancy_id, display_name=AMD_NAME).data
    active = [i for i in existing if i.lifecycle_state not in ("TERMINATED", "TERMINATING")]
    if active:
        inst = active[0]
        print(f"Reusing existing AMD instance: {inst.id}")
    else:
        image_id = find_ubuntu_x86_image(compute, tenancy_id)
        print(f"Image:  {image_id}")
        print("Launching VM.Standard.E2.1.Micro (1/8 OCPU, 1 GB RAM)...")
        inst = compute.launch_instance(oci.core.models.LaunchInstanceDetails(
            availability_domain=ad,
            compartment_id=tenancy_id,
            display_name=AMD_NAME,
            shape="VM.Standard.E2.1.Micro",
    ...
    state["amd_public_ip"] = public_ip
    state["amd_private_ip"] = private_ip
