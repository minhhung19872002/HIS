...
using HIS.Application.DTOs.SampleBatch;
...
/// Tách khỏi SampleBatchController (#202 thin-controller). Behavior-preserving: mọi query/
/// projection/partition/response shape + message giữ nguyên; return map về ServiceOutcome.
/// </summary>
public class SampleBatchService : ISampleBatchService
{
    private readonly HISDbContext _db;
    public SampleBatchService(HISDbContext db) { _db = db; }

    public async Task<ServiceOutcome> GetBatchesAsync(DateTime? date)
    {
        ...
        return ServiceOutcome.Ok(new BatchReportDto(target, batches, shaped.Count));
    }
}
