...
/// <summary>
/// DEV helper: link today's RadiologyRequests with real Orthanc StudyInstanceUIDs so the
/// worklist page shows "has images" + DicomViewer can load real DICOM data. Needed because
/// PopulateData seeds requests but doesn't attach DICOM metadata.
/// Moved verbatim out of DevLinkRadiologyController (#365 REFAC-3).
/// </summary>
public class DevLinkRadiologyServiceImpl : IDevLinkRadiologyService
{
    private readonly HISDbContext _db;
    private readonly IConfiguration _config;

    public DevLinkRadiologyServiceImpl(HISDbContext db, IConfiguration config)
    {
        _db = db;
        _config = config;
    }

    // Shims mirroring ControllerBase.Ok()/BadRequest() so the method body moved verbatim
    // from DevLinkRadiologyController keeps compiling unchanged.
    private static DevActionResult Ok(object value) => new(200, value);
    private static DevActionResult BadRequest(object value) => new(400, value);

    public async Task<DevActionResult> LinkTodayAsync()
    {
        ...
        // Original returned the strongly-typed `LinkResult` record
        // (HIS.API.Dtos.DevLinkRadiology) — kept in the API/controller layer since
        // HIS.Infrastructure cannot reference HIS.API. Same property names/order so the
        // controller reconstructs an identical LinkResult / identical JSON shape.
        return Ok(new DevLinkRadiologyResult(requestsUpdated, examsCreated, studiesCreated, uids));
    }
}
