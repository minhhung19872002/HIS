...
/// </summary>
public partial class PopulateDataServiceImpl : HIS.Application.Services.IPopulateDataService
{
    private readonly HISDbContext _db;
    private readonly ILogger<PopulateDataServiceImpl> _logger;
    public PopulateDataServiceImpl(HISDbContext db, ILogger<PopulateDataServiceImpl> logger)
    {
        _db = db;
        _logger = logger;
    }

    // Shim mirroring ControllerBase.Ok(object) so the method bodies moved verbatim
    // from PopulateDataController (which used `return Ok(...)`) keep compiling
    // unchanged — the value just flows straight through as the raw DTO/object.
    private static object Ok(object value) => value;
    ...
    // INFECTION CONTROL
    // ==========================================================================
    public async Task<object> PopulateInfectionControlAsync()
    {
    ...
    // PATIENT PORTAL
    // ==========================================================================
    public async Task<object> PopulatePatientPortalAsync()
    {
        var ctx = await LoadCtxAsync();
        var summary = new Dictionary<string, int>();
    ...
    // ==========================================================================
    public async Task<object> PopulateEquipmentAsync()
    {
        var ctx = await LoadCtxAsync();
        var summary = new Dictionary<string, int>();
    ...
    // PATHOLOGY
    // ==========================================================================
    public async Task<object> PopulatePathologyAsync()
    {
        var ctx = await LoadCtxAsync();
        var summary = new Dictionary<string, int>();
    ...
    // ==========================================================================
    public async Task<object> PopulateQualityAsync()
    {
        var ctx = await LoadCtxAsync();
        var summary = new Dictionary<string, int>();
