$token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..PLjZGiCYaqhrABT2taNgQC2WSIy9pgDGohlhA2tYcRI"
$headers = @{Authorization = "Bearer $token"}
$medicalRecordId = "704b09aa-cf75-4102-8bc0-2f8ed5023583"
$patientId = "9412e970-cd28-4c46-8cb6-0321cc30ebfa"

Write-Host "=== Step 7: Calculate Invoice ===" -ForegroundColor Cyan
try {
    $invoice = Invoke-RestMethod -Uri "http://localhost:5106/api/billingcomplete/invoices/calculate/$medicalRecordId" -Headers $headers
    $invoice | ConvertTo-Json -Depth 5
} catch {
    Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Red
    $_.Exception.Response
}
