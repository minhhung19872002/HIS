...
| `biz` | Business Domain | nghiệp vụ HIS review — clinical workflow validation, business rule audit, BHYT/Bộ Y Tế compliance check TRƯỚC khi code |
...
| **HIS DOMAIN REVIEW — code task chạm clinical module** (`modules/opd`, `modules/billing`, `modules/emr`, `modules/laboratory`, `modules/inpatient`, hoặc backend service tương ứng) | `his-biz-reviewer` + `his-biz-<module>` (xem `skill-routes/biz.md` cho module-to-skill mapping) — MANDATORY FIRST STEP: chạy Phase 4 assessment → confidence ≥ 80% mới proceed; < 80% → STOP + ask. **Cũng scan proactively** trong lúc đọc code: nếu phát hiện sai/thiếu nghiệp vụ ngoài phạm vi task → cảnh báo + đề xuất fix direction. Không bỏ qua bước này kể cả khi task nhỏ/bug fix. |
