...
## Khi nào KHÔNG dùng
- CRUD nghiệp vụ thường không liên quan thanh toán → `his-backend-module-scaffold`.
- Page list/detail thuần → `his-frontend-page-v2` (BankPayments UI vẫn có thể dùng kèm).
...
3. **FE**: BankPayments hiển thị giao dịch + QR (img.vietqr.io preview) + modal confirm. Theo `his-frontend-page-v2` cho layout.
...
- Hàm này **dùng chung** cho VNPay/MoMo/ZaloPay/VietQR → sửa nó phải **regression cả 4 cổng** (xem `his-e2e-testing`).

## Pitfalls
- **CRC sai** → app NH không scan được QR / BN chuyển nhầm. Tính CRC trên đúng chuỗi tới `6304`.
- **Hardcode số TK BV** → dùng appsettings/env (`his-anti-pattern`).
