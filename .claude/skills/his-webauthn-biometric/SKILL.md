...
## Quy trình chuẩn
1. **API client**: thêm/sửa hàm trong `api/nangcap24.ts` object `biometric` (begin/finish, list, revoke) — theo `his-api-client`.
...
- Test E2E: WebAuthn KHÔNG chạy headless → đánh dấu `skip` hoặc test bằng virtual authenticator (CDP). Xem `his-e2e-testing`.
