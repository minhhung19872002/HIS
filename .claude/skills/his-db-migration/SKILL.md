...
- **Bug `InvalidCastException Guid↔String` (đã fix 31 bảng)**: khi DB là `UNIQUEIDENTIFIER` mà C# entity khai `string?` → cần ValueConverter global trong `HISDbContext`. Khi tạo bảng mới với `CreatedBy UNIQUEIDENTIFIER`, **PHẢI thêm tên bảng vào whitelist `tablesWithGuidAudit`** trong `HISDbContext` (đây là HashSet áp ValueConverter Guid↔String cho CreatedBy/UpdatedBy) — xem `audit-columns-convention.md` mục "Đăng ký ValueConverter". Quên = 500 InvalidCastException khi query bảng đó.
...
## Seed / populate demo data (chuẩn bị demo khách)

Nhiều trang lọc `CreatedAt.Date == today` → data để ngày cũ thì trang rỗng. Pattern đã dùng:
- **Idempotent seed**: mọi `PopulateX`/seed bọc `IF NOT EXISTS` hoặc `!await db.X.AnyAsync()` để chạy lại an toàn.
- **`PopulateDataController`** (`/api/admin/populate/all`) + **DailySeed** (`/api/admin/seed-daily/patients`,
  header `X-Seed-Key`) — gọi API thật để fill data, KHÔNG cần deploy. POST phải có body `-d '{}'` (Google LB
  trả 411 nếu rỗng).
- **Shift-to-today**: bump slice mới nhất của `MedicalRecords/Examinations/ServiceRequests/Prescriptions/
  QueueTickets/LabOrders/Receipts` sang hôm nay. ⚠️ **Container chạy UTC** → `SYSDATETIME()` trong SQL trả UTC,
  lệch giờ host. Khi shift "to today" cho demo dùng **chuỗi ngày VN tường minh** (vd `'2026-04-26'`), KHÔNG
  `SYSDATETIME()`. Dùng `CAST(CAST(SYSDATETIME() AS date) AS datetime2)` khi cần DATEADD phút.
- **Drift guard**: bọc UPDATE bằng `COL_LENGTH(...) IS NOT NULL` để bỏ qua cột không tồn tại (BAK local vs prod lệch schema).
- **KHÔNG seed data giả nếu user yêu cầu data thật** từ DB gốc.
