...
## Khi nào KHÔNG dùng (route sang skill chuyên biệt)

- Service gọi **cổng HTTP ngoài** (cổng QG/BHXH/Zalo/FHIR/SMS) → `his-be-external-gateway`.
- **BackgroundService/hosted worker** chạy nền theo chu kỳ → `his-be-background-worker`
  (đừng inject DbContext vào ctor — sẽ ObjectDisposedException).
- **SignalR Hub / realtime push** → `his-fs-realtime-signalr`.
- Service thanh toán → `his-be-payment-gateway`. Bảng SQL → `his-db-migration`.
...
```
Test endpoint mới qua PowerShell (xem skill `his-test-api-powershell`).
...
```
**Đọc `audit-columns-convention.md` của skill `his-db-migration`** để biết bảng đang dùng Guid hay Nvarchar cho CreatedBy.
...
- **Migration không apply**: nếu thêm bảng qua EF migration mà CLAUDE.md ghi "PendingModelChangesWarning Ignore" — project đang IGNORE pending model changes (xem `DependencyInjection.cs:28`). Phải tạo SQL script tay (xem skill `his-db-migration`) thay vì trông cậy `dotnet ef migrations`.
