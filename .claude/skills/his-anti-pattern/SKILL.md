...
1. **Quên đăng ký DI** service/controller mới trong `DependencyInjection.cs` → **500 runtime** không stack trace rõ. Luôn kiểm tra DI đầu tiên khi 500. (xem `his-backend-module-scaffold`)
...
3. **`dotnet ef migrations`** trông cậy auto-apply — dự án IGNORE pending model changes. Phải viết SQL script tay đánh số `NN_*.sql` (xem `his-sql-table-migration`).
...
8. **Login qua form UI trong test** → chậm/flaky. Dùng API token + set localStorage (xem `his-e2e-testing`).
9. **`console.error` cho lỗi API kỳ vọng** → fail smoke `console-errors.cy.ts`. Dùng `console.warn`.
10. **Antd deprecated props** (`Space direction`, `Alert message`, `Drawer width`, `destroyOnClose`...) → dùng API v6 (xem `his-antd-v6`).
...
13. **Push code BE rồi tưởng đã deploy** — Cloud Run KHÔNG auto-deploy. Phải `gcloud builds submit` + `run services update` (xem `his-deploy`).
