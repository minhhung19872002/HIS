...
| I.2.5.8 | **Xem file kết quả xét nghiệm trên app** | Bảng chỉ số trong app thay cho tệp PDF; `ReportUrl` trong DTO trỏ `api/portal/lab-results/{id}/pdf` để bản in dùng sau | ⬜ Chưa làm — xem [D11](decisions.md): app hiện **bảng chỉ số gốc** (đọc được, phóng to được, đúng thứ HSMT đòi "xem file kết quả") thay vì nhúng PDF; endpoint PDF để Phase 7 làm cùng phần in ấn | 3 | ⬜ |
