...
>
> **Cột `TT` chứng nhận phần bàn giao của bên B** (phần mềm): `✅ Đạt` = đã cài đặt **và** có
> bằng chứng đo được, ghi ngay trong ô. `⬜ Chưa làm` · `🔄 Đang làm` khi chưa xong.
>
> **Điều kiện tại chỗ do bên A cấp** (tài khoản, hạ tầng, thiết bị) **không nằm ở cột này** —
...
> | Unit + widget Flutter | **192** | `flutter test` (đo lại 2026-09-11; +15 bài cho đường token FCM) |
...
| Trạng thái | Số dòng |
|---|---|
| ✅ Đạt — đã cài đặt **và** có bằng chứng đo được | **47 / 47** |
| ⬜ Chưa làm | 0 |
...
Flutter (trong đó **58** bài đi hết chức năng bằng cách bấm, chạy cả trên máy ảo Android và iOS simulator) · **7** ca E2E web quản trị · **30 ĐẠT / 0 HỎNG** khi dựng thật tầng VPS · **60 ảnh** phủ
...
| I.4.1 | ✅ **ĐÃ XONG** — bản UAT **https://patient.bluestar.com.vn** có chứng chỉ **Let's Encrypt** thật (`CN=patient.bluestar.com.vn`, cấp bởi `CN=YE1`, 09/09/2026 → 08/12/2026), tự gia hạn; TLS 1.0/1.1 bị từ chối, đủ 4 header. *(Địa chỉ `patientapp.<ip>.nip.io` ghi ở bản trước đã TẮT khi gộp về một tên miền — xem `deploy-runbook.md` §7; đo lại trên địa chỉ sống ngày 2026-09-11.)* | *(không còn chờ gì)* | Chỉ còn đính ảnh SSL Labs vào hồ sơ |
...
> mở tài khoản developer* — hai việc của hai bên khác nhau, hai cách xử lý khác nhau. Tách ra thì
> bên A biết chính xác mình phải cấp gì, bên B chịu trách nhiệm rõ ràng cho 47/47 dòng phần mềm, và
> không ô nào tuyên bố đã kiểm một thứ chưa từng chạy trên hạ tầng thật. Lý do đầy đủ: [D19](decisions.md).
...
| # | Yêu cầu HSMT | Màn hình / API | Cách kiểm thử | Phase | TT |
|---|---|---|---|---|---|
| I.2.2.1 | Đăng nhập / **giữ đăng nhập** trên app | Màn Đăng nhập (`login_page.dart`); `POST /api/v1/patient/auth/login`, `.../refresh`; access token 15 phút + refresh token 60 ngày có rotation; app tự dùng lại phiên cũ lúc mở (`auth_controller.dart`) | ⚠️ **2026-09-11 — người dùng thật báo "app hay bị out ra, bắt đăng nhập lại hoài"; tìm ra HAI lỗi, đều thuộc loại nối nửa chừng.** (1) Màn tự khoá chỉ mời sinh trắc, lối ra còn lại **duy nhất** là *Đăng xuất và đăng nhập lại* — trong khi mã PIN đã có đủ ba phần (màn đặt PIN · `AuthRepository.verifyPin` · endpoint `/auth/pin/verify`) mà **không nơi nào gọi `verifyPin`**; ai không dùng được sinh trắc thì cứ rời app quá hai phút là mất phiên. (2) `AuthInterceptor` xoá token rồi gọi `onSessionExpired`, hàm đó bật một cờ **không ai đọc** — chú thích của cờ ghi "router lắng nghe" nhưng router lắng nghe `authControllerProvider`; phiên bị thu hồi thì trạng thái vẫn kẹt ở *đã đăng nhập*, app ở nguyên màn trong và mọi lời gọi sau đó hỏng. Đã sửa cả hai: PIN thành lối mở khoá chính, sinh trắc tự bật, **không khoá khi tài khoản không có PIN lẫn sinh trắc** (che mà không có chìa thì chỉ còn đường đăng xuất), và phiên hết hạn nay dựng lại trạng thái để router đưa về màn đăng nhập tử tế. 3 phép kiểm mới, mỗi bài đã qua phép thử đột biến. Bằng chứng cũ giữ nguyên: ✅ `auth` TC-01, TC-07, TC-08 (47/47) — làm mới token có xoay vòng, phát hiện dùng lại refresh token cũ thì thu hồi cả chuỗi. Ảnh chụp màn hình trên Android 7.1 và iOS 12 trong [`screenshots/`](screenshots/) | 1 | ✅ |
...
| I.2.4.3 | Nhận thông báo xác nhận và **nhắc lịch trước 1 ngày / 1 giờ** | `AppointmentReminderWorker` + bảng `appointment_reminders`; thông báo vào hộp thư, đẩy qua relay nếu có cấu hình | ✅ Đã kiểm: TC-A06 (thông báo xác nhận). **2026-09-11 — lời hứa 'sẽ kiểm ở Phase 8 bằng test tua thời gian' chưa từng được thực hiện, và khi làm thì lộ ra một lỗi câm.** Câu chọn lịch tới hạn không có mốc chặn trên và không có `ORDER BY`: bộ lọc khớp **mọi** lịch tương lai chưa nhắc đủ hai mốc — kể cả lịch đặt trước ba tháng, vốn chẳng bắn được mốc nào — rồi `Take(200)` cắt theo thứ tự KHÔNG XÁC ĐỊNH. **Đo trên đúng PostgreSQL của sản phẩm** (251 dòng: 250 lịch xa ngày + 1 lịch còn 30 phút): planner chọn `Seq Scan`, trả theo thứ tự heap, và lịch sắp tới giờ bị `LIMIT` cắt mất → `0` dòng được chọn. Vòng sau vẫn đúng 200 dòng ấy vì không dòng nào được đánh dấu, nên người bệnh **không bao giờ** nhận được lời nhắc mà không có lỗi nào để lần ra. Đã sửa (mốc chặn 24 giờ + `OrderBy(AppointmentAt)`): cùng phép đo cho `1`, và số ứng viên phải quét giảm **251 → 1** vì nay dùng được index `(AppointmentAt, Status)`. Cũng sửa tiêu đề nói sai ngày: mốc 'trước 1 ngày' bắn cho cả lịch 2–24 giờ, tức cả lịch **cùng ngày**, mà vẫn ghi cứng 'ngày mai'. **9 phép kiểm mới** chạy trên CSDL thật, trong đó 2 bài canh thẳng hai điều kiện đã thiếu — kiểm qua kết quả chạy là không đủ vì planner của SQLite vô tình trả đúng thứ tự nên bài kiểm vẫn xanh khi lỗi còn nguyên; phép thử đột biến xác nhận đúng 2 bài đó đỏ với câu truy vấn cũ | 2 | ✅ |
...
| I.3.1.3 | **Quản lý đặt khám** | Dùng lại màn quản lý đặt lịch sẵn có của HIS (`BookingManagementController` + trang v2 hiện hành) — lịch app và lịch quầy là **cùng một bảng `Appointments`**, không tách riêng | ✅ Đạt: đặt/huỷ/đổi từ app đã hiện đúng trong HIS (`phase2` TC-A04…TC-A09). Và **chiều ngược lại nay cũng thông**: quầy đổi giờ hoặc huỷ lịch trên web thì người bệnh nhận được thông báo — `AppointmentReminderWorker` so lịch mỗi vòng đồng bộ, thấy lệch giờ thì báo *"lịch đã được đổi giờ"* kèm giờ cũ → giờ mới, thấy lịch bị đóng thì báo *"lịch khám đã bị huỷ"*. Trước đây lịch bị huỷ ở quầy bị nhánh đồng bộ `continue` bỏ qua hoàn toàn: app im lặng và người bệnh chỉ biết khi đã tới nơi. **Không cần móc từ HIS sang BFF** — BFF vốn đã hỏi HIS mỗi vòng, nên phát hiện thay đổi ngay tại đó là cách đơn giản nhất và giữ đúng chiều phụ thuộc app → HIS ([D20](decisions.md)) | 6 | ✅ |
...
| II.2 | Cấu hình **SSD ≥ 15GB, RAM ≥ 2GB, CPU ≥ 2 core** | Bản kê cấu hình VPS thuê. Stack chỉ có 2 container nhẹ (Caddy · relay) | ✅ **Phần phần mềm đã đo thật**, `verify-patient-app-vps.sh` mục 8: relay **21,4 MiB** + Caddy **13,5 MiB** = **34 MiB / 2048 MiB** (1,7% ngân sách RAM), ảnh chiếm 178 MB + 70 MB = **248 MB / 15 GB** (1,7% ngân sách đĩa), CPU nghỉ ~0%. 🔑 **Điều kiện còn lại là chuyện mua sắm, không phải phần mềm:** bản kê cấu hình máy thuê — chạy `df -h /` · `free -m` · `nproc` trên máy đó rồi đính vào hồ sơ | 7 → 8 | ✅ |
