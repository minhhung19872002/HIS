...
| **8** ✅ | **TEST** (bắt buộc, **luôn đi cuối** theo `CLAUDE.md`) | ✅ **67 unit test backend** cho BFF, chạy trong CI gate `dotnet test` (ví giấy tờ · so phiên bản · chuẩn hoá SĐT · mật khẩu tạm · giờ VN) · ✅ **92 test Flutter** (từ 12): đọc JSON máy chủ, cờ bất thường, cổng buộc cập nhật, tự khoá · ✅ **7 ca E2E Playwright** cho 5 màn quản trị · ✅ **30 ảnh bằng chứng** phủ đủ từng nhóm chức năng I.2 + I.3 **và các trạng thái lỗi · rỗng · nhập thiếu · đang tải** (quy ước evidence §3 đòi đủ trạng thái, không riêng đường vui), chụp trên máy ảo **Android 7.1.1 (API 25)** — đúng ngưỡng HSMT — và đưa vào trình xem evidence (`spec-app/`, dữ liệu ở `data/15-app.js`). **Ba lỗi thật tìm được nhờ đợt test này**: chỉ số xét nghiệm bị tô đỏ nhầm khi cờ rỗng · đăng nhập sinh trắc chết hẳn trên Android (`FlutterActivity` thay vì `FlutterFragmentActivity`) · thiếu proxy `/patient-api` nên 5 màn quản trị trắng ở môi trường dev |
