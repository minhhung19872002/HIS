...
| **8** ✅ | **TEST** (bắt buộc, **luôn đi cuối** theo `CLAUDE.md`) | ✅ **67 unit test backend** cho BFF, chạy trong CI gate `dotnet test` (ví giấy tờ · so phiên bản · chuẩn hoá SĐT · mật khẩu tạm · giờ VN) · ✅ **92 test Flutter** (từ 12): đọc JSON máy chủ, cờ bất thường, cổng buộc cập nhật, tự khoá · ✅ **7 ca E2E Playwright** cho 5 màn quản trị · ✅ **60 ảnh bằng chứng** — 30 trên máy ảo **Android 7.1.1 (API 25)** + 30 tương ứng trên **iOS 12 simulator**, phủ đủ từng nhóm chức năng I.2 + I.3 **và các trạng thái lỗi · rỗng · nhập thiếu · đang tải** (quy ước evidence §3 đòi đủ trạng thái, không riêng đường vui) — đúng ngưỡng HSMT — và đưa vào trình xem evidence (`spec-app/`, dữ liệu ở `data/15-app.js`). **Ba lỗi thật tìm được nhờ đợt test này**: chỉ số xét nghiệm bị tô đỏ nhầm khi cờ rỗng · đăng nhập sinh trắc chết hẳn trên Android (`FlutterActivity` thay vì `FlutterFragmentActivity`) · thiếu proxy `/patient-api` nên 5 màn quản trị trắng ở môi trường dev |

| **9** ✅ | **Rà soát lại bảng nghiệm thu đối chiếu mã nguồn** (2026-09-11) | ✅ Soi lại từng dòng HSMT tới tận `file:line` thay vì tin ô đã ghi ✅. 46/47 khớp; **1 dòng không khớp: I.2.2.2 thông báo đẩy** — nửa đường ống phía app chưa bao giờ được nối (`PushTokenRegistrar` không ai gọi · `token()`/`tokenRefreshes`/`deepLinks` không ai đọc · plugin Gradle `google-services` không apply · iOS thiếu `UIBackgroundModes` + entitlement `aps-environment`), nên `Devices.PushToken` luôn NULL và outbox luôn rỗng. Đã vá đủ 5 mắt xích + `PushRegistration` + nối deep-link cho cả hộp thư trong app; **15 phép kiểm mới** (2 bài dựng app thật, đã chứng minh bằng phép thử đột biến là gỡ dòng nối thì đỏ). **Đợt rà thứ hai (sâu hơn)** tìm thêm **lỗi câm thứ hai**: câu chọn lịch tới hạn của `AppointmentReminderWorker` thiếu mốc chặn trên và thiếu `ORDER BY` → đo trên đúng PostgreSQL của sản phẩm thì `Seq Scan` + `LIMIT 200` cắt mất lịch sắp tới giờ, người bệnh không bao giờ được nhắc. Đã sửa + **9 phép kiểm mới chạy trên CSDL thật** (trước đó 67 phép kiểm của BFF đều chỉ chạm hàm thuần, **không bài nào chạm CSDL**). Đối chiếu hợp đồng toàn bộ: 28/28 đường BFF→HIS khớp · 0 provider chết · 0 stub trong BFF · 2 worker còn lại đều có `OrderBy` + vị từ chặn · 14/14 endpoint kết quả đều qua đúng một cửa kiểm quyền xem hộ. **Đợt rà thứ ba + báo lỗi từ người dùng thật** ("app hay bị out ra, bắt đăng nhập lại hoài") tìm thêm **ba lỗi nữa, cùng một họ nối-nửa-chừng**: (a) màn tự khoá chỉ mời sinh trắc, lối ra còn lại duy nhất là đăng xuất — `verifyPin` có sẵn mà không nơi nào gọi; (b) `onSessionExpired` bật một cờ không ai đọc nên phiên bị thu hồi thì app kẹt trong màn trong; (c) relay gửi khoá APNs `contentAvailable` thay vì `content-available`, nên iOS không đánh thức app chạy ngầm mà không có lỗi nào. Đã sửa cả ba + 6 phép kiểm mới, mỗi bài qua phép thử đột biến. Rà thêm và sạch: 177/177 khoá JSON app↔server khớp · mọi section cấu hình đều được bind · không trôi migration (`dotnet ef migrations has-pending-model-changes`). Gate: `flutter analyze` sạch · `flutter test` **192 PASS** · `dotnet test` **360 PASS** · `flutter build apk --debug` PASS cả khi **có** lẫn **không có** `google-services.json` |
...
build ra** = **12.0**. Chạy **58/58 bài đi-hết-chức-năng** (bấm thật từng màn trên simulator) và chụp **30 màn** trên iPhone simulator.
