# [ARCHIVE] STATUS snapshot 2026-06-13 → 2026-06-21

> Lịch sử phiên cũ, tách khỏi STATUS.md khi siết về bản ngắn (2026-06-22). Trạng thái SỐNG hiện tại: [`../../STATUS.md`](../../STATUS.md). Nội dung dưới giữ nguyên-văn để truy lịch sử.
...
> Cập nhật cuối: **2026-06-21** — GOVERNANCE **dedup `.claude` (nhóm 1+2) = ✅ DONE · LINT OK · chưa commit/push**:
> ① REGISTRY +5 owner-row cho rule mồ côi (test-last · model-tier · task-lifecycle · SYNC-GATE · evidence-viewer). ② SKILL-MAP §0c (git-ops bảng) + §2 (build-gate/self-review row) **co về link**. ③ `project-rules.md` §3/§4 thành owner git-ops đầy đủ (3 mức keyword + edge-case). `bash .claude/lint.sh` = **LINT OK ✅ 0 cảnh báo**. (D4 his-qa #28-29 BỎ — là code-placement ≠ §0a report-placement, dương-tính-giả.)
>
> Cập nhật cuối: **2026-06-21** — TECH-DEBT **#195 [PERF-1] AsNoTracking + fix N+1 = 🟡 ĐANG LÀM (batch 1-4 SAFE done · money/kho/safety DEFER · chưa commit/push · in-progress)**:
> **Batch 1** `PdfGenerationService.cs` (read-only thuần, 0 SaveChanges): 16 `AsNoTracking` entity + **fix 4 N+1** (batch-load dict: Users theo DoctorId/NurseId ×3 + ServiceRequestDetailParameters ×1) — dict-lookup ≡ per-item query; include ≤1 collection (không cartesian). **Batch 2** `PublicEmrLookupService.cs`: 2 `AsNoTracking` query entity (bỏ 7 projection). **Batch 3** `FhirService.cs` (read-only thuần, **0 N+1 thật** — Explore báo sai, đã verify 3 foreach chỉ gọi map thuần): 24 `AsNoTracking` cho 24 query-root entity (chừa L590 `.Any()` subquery — thêm vào sẽ lỗi expression-tree runtime). **Batch 4** (read-only services, AsNoTracking entity-only, bỏ projection theo "cái nào cần"): `ReceptionCompleteService.Printing`(6) · `BillingCompleteService.Printing`(9, **chừa Patients.FindAsync** vì FindAsync không chain AsNoTracking) · `ResultNotificationService`(2) · `DataInheritanceService`(5 entity, bỏ 9 projection/scalar) · `ExaminationCompleteService.Reports`(4); `Cv365XmlService` sẵn-có. Cả 4 batch: **build HIS.Infrastructure EXIT 0 · 0 Error** (946 warning nullable pre-existing); spot-check diff chỉ-thêm-AsNoTracking, 0 đổi logic/shape; guard 0 `AsNoTracking().FindAsync/.Any`.
> **KẾT LUẬN scope (tech-lead, honest):** phần PERF-1 CÒN LẠI = **N+1 trong write-path vùng tiền/kho/patient-safety** (Warehouse StockIn/StockOut/Inventory · BloodBank · PharmacyApproval · Examination/Inpatient Prescriptions+ServiceOrders · Reception OrdersBilling · Billing LockApprove) → **DEFER** theo SAFETY-PROTOCOL #3 (cần characterization-test + deploy/smoke; máy này KHÔNG smoke prod). Phần report/projection → **SKIP** (AsNoTracking vô ích). #195 GIỮ OPEN + `in-progress`; comment #195 = inventory N+1 deferred chi tiết.
>
...
> **Verify mỗi file:** `npm run build` (tsc -b + vite) **EXIT 0** · export-count khớp HEAD · **0 caller** đổi (pages/pages-v2/pages-mobile/components sạch) · spot-check pure-move.
...
>
> **Việc kế tiếp:** ~~(a) nhóm endpoint còn lại cần fix sâu~~ ✅ **DONE** (`60a6d2e` guard 9 endpoint TbHiv/IvfLab/HospitalPharmacy + MentalHealth/Pathology/TraditionalMedicine/Trauma/PharmacyApproval/Forensic; Notification/test=no-body skip; **verify prod 9/9 `{}`→400**); (b) **P2 dọn data rác prod = ✅ DONE** (2026-06-18): connect prod sqlcmd → SELECT-count → look-before-delete → xóa **7 record rác + 1 child FK**, verify REMAINING=0 (`scripts/cleanup_qa_garbage_2026-06-17.sql`). 🔴 **FINDING bảo mật:** Cloud SQL authorizedNetworks=`0.0.0.0/0` (prod mở internet) → issue #292 siết IP, không tự sửa (Cloud Run dùng public IP); (c) LIS P1#2 (`medicalRecordId` LabOrderDetailDto) — đã ship trong `4212eef`.
>
