...
>
> Cập nhật cuối: **2026-07-12** — **cửa components-restructure: FINAL ✅ PUSHED (commit này — xóa 41 shim gốc `components/*.tsx` + move `layouts/`→`components/layout/` + `guards/`→`router/guards/` + `components/permission`→`router/permission` + 112 file import-fix + xóa 33 `modules/*/permissions/.gitkeep` theo chỉ đạo user + fix import `layouts/terminal` trong các trang origin của batch-3/4); wave-7 ✅ `f66c068`; wave-6 ✅ `07facba`; wave-5 ✅ `0f27111`; wave-4 ✅ `287a711`. cửa 367-authz1: ✅ `e3c6d24` (#367 OPEN — FE gating defer). cửa 368-authz2: ✅ `edcbb78`. cửa 409: batch-2 ✅ `5217c77` · batch-3 ✅ `f594867` · batch-4 ✅ `b15b4e8`. ★ LƯU Ý cửa 409: mọi WIP local còn import `layouts/terminal|components/<Shim>` phải đổi sang `components/layout/terminal` / nhà mới trong `modules/*/components` trước khi push (path cũ đã xóa ở commit components-FINAL).**
