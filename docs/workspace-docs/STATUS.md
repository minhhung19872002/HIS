...
>
> **Việc kế tiếp:** ~~(a) nhóm endpoint còn lại cần fix sâu~~ ✅ **DONE** (`60a6d2e` guard 9 endpoint TbHiv/IvfLab/HospitalPharmacy + MentalHealth/Pathology/TraditionalMedicine/Trauma/PharmacyApproval/Forensic; Notification/test=no-body skip; **verify prod 9/9 `{}`→400**); (b) **P2 dọn data rác prod** — script guarded SELECT-count+DELETE đã viết `scripts/cleanup_qa_garbage_2026-06-17.sql` (manual-run, KHÔNG auto-apply); **chưa chạy** (cần phiên DB: `CreatedAt` không có trong API → phải SQL prod; chạy Part A đếm trước rồi Part B xóa); (c) LIS P1#2 (`medicalRecordId` LabOrderDetailDto) — đã ship trong `4212eef`.
>
