...
let totalTables = 0;
BANDS.forEach((b) => b.modules.forEach((m) => (totalTables += m.tables.length)));

// stats
$("#stats").innerHTML = [
 ["485","DbSet (bảng) trong DbContext"],[totalTables,"bảng trong sơ đồ"],
 [BANDS.reduce((a,b)=>a+b.modules.length,0),"phân hệ"],["5","lớp kiến trúc"],
 ["SQL Server","CSDL"]
].map((s)=>`<div class="stat"><b>${s[0]}</b>${s[1]}</div>`).join("");
$("#fcount").textContent = totalTables;

// legend
$("#legend").innerHTML = BANDS.map((b)=>`<span><i style="background:${b.color}"></i>${b.title}</span>`).join("");

// flow
$("#flow").innerHTML = FLOW.map((f,i)=>
 `<div class="fstep"><span class="fi">${f[0]}</span>${f[1]}</div>`+
 (i<FLOW.length-1?'<span class="farrow">➜</span>':'')).join("");

// toc
$("#toc").insertAdjacentHTML("beforeend",
 BANDS.map((b)=>`<span class="nl" data-go="band-${b.id}" style="border-color:${b.color}">${b.title}</span>`).join(""));

// master
$("#master").innerHTML = BANDS.map((b)=>`
 <section class="band" id="band-${b.id}">
   <div class="band-h"><span class="dot" style="background:${b.color}"></span>
     <h2 style="color:${b.color}">${b.title}</h2><span class="bt">${b.sub}</span></div>
   <div class="grid">
     ${b.modules.map((m)=>`
       <div class="mod" data-go="d-${b.id}-${m.id}" data-search="${(m.nm+' '+m.tables.map((x)=>x[0]+' '+x[1]).join(' ')).toLowerCase()}"
            style="border-top-color:${b.color}">
         <div class="mod-h"><span class="ic">${m.ic}</span><span class="nm">${m.nm}</span>
           <span class="cnt">${m.tables.length} bảng</span></div>
         <div class="desc">${m.desc}</div>
         <div class="chips">
           ${m.tables.slice(0,7).map((x)=>`<span class="chip">${x[0]}</span>`).join("")}
           ${m.tables.length>7?`<span class="more">+${m.tables.length-7} bảng…</span>`:""}
         </div>
       </div>`).join("")}
   </div>
 </section>`).join("");
...
// details
$("#details").innerHTML = BANDS.map((b)=>b.modules.map((m)=>`
 <div class="detail" id="d-${b.id}-${m.id}" style="border-left-color:${b.color}"
      data-search="${(m.nm+' '+m.tables.map((x)=>x[0]+' '+x[1]).join(' ')).toLowerCase()}">
   <div class="detail-h"><span class="ic">${m.ic}</span><span class="nm">${m.nm}</span>
     <span class="tag" style="background:${b.tag}">${b.title.split('—')[0].trim()}</span>
     <span class="cnt">${m.tables.length} bảng</span></div>
   <div class="desc">${m.desc}</div>
   <div class="tbl-grid">
     ${m.tables.map((x)=>`<div class="trow"><span class="vn">${x[1]}</span><span class="tn">${x[0]}</span></div>`).join("")}
   </div>
   ${m.rel?`<div class="rel">🔗 <b>Quan hệ chính:</b> ${m.rel}</div>`:""}
 </div>`).join("")).join("");

/* ===== (A) OVERVIEW — quan hệ 5 lớp (khối + mũi tên có nhãn) ===== */
function renderOverview(){
  const host = document.getElementById("overview"); if(!host) return;
  host.innerHTML = `<div class="ov"><svg class="ov-svg"></svg><div class="ov-grid">
    ${LAYER_REL.nodes.map((n)=>`<div class="ov-node" data-id="${n.id}" style="grid-area:${n.area};--nc:${LAYERC[n.id]}">
        <div class="ic">${n.ic}</div><div class="t">${n.t}</div><div class="d">${n.d}</div></div>`).join("")}
  </div></div>`;
}
function drawOverview(){
  const ov = document.querySelector("#overview .ov"); if(!ov) return;
  const svg = ov.querySelector(".ov-svg");
  ...
  const edgePt = (s,tx,ty)=>{const dx=tx-s.cx,dy=ty-s.cy;const k=Math.min(s.hw/Math.max(Math.abs(dx),1e-3),s.hh/Math.max(Math.abs(dy),1e-3));return{x:s.cx+dx*k,y:s.cy+dy*k};};
  ...
  LAYER_REL.edges.forEach((e)=>{
    const ea=ov.querySelector(`.ov-node[data-id="${e.a}"]`), eb=ov.querySelector(`.ov-node[data-id="${e.b}"]`);
    if(!ea||!eb) return;
    const A=R(ea), B=R(eb);
    const p1=edgePt(A,B.cx,B.cy), p2=edgePt(B,A.cx,A.cy);
    out += `<path d="M ${p1.x} ${p1.y} L ${p2.x} ${p2.y}" marker-end="url(#ah)"/>`;
    const mx=(p1.x+p2.x)/2, my=(p1.y+p2.y)/2, w=Math.min(e.l.length*5.7+14, 230);
    out += `<rect class="elabel-bg" x="${mx-w/2}" y="${my-9}" width="${w}" height="18" rx="5"/>`;
    out += `<text class="elabel" x="${mx}" y="${my+3.6}" text-anchor="middle">${e.l}</text>`;
  });
  svg.innerHTML = out;
}

/* ===== (B) ROADMAP — cơ chế hoạt động (spine + nhánh + vai trò) ===== */
function renderRoadmap(){
  const host = document.getElementById("roadmap"); if(!host) return;
  const chips = (arr,side)=>arr.map((x)=>
    `<div class="rm-child l-${x.layer}${x.opt?' opt':''}" data-go="${x.ref}" data-stage="${x._sid}" data-side="${side}">${x.label}${x.opt?' <em>(tùy chọn)</em>':''}</div>`).join("");
  const rows = STAGES.map((s,i)=>{
    s.children.forEach((c)=>c._sid=s.id);
    const half=Math.ceil(s.children.length/2);
    const L=s.children.slice(0,half), Rr=s.children.slice(half);
    return `<div class="rm-row">
      <div class="rm-side left">${chips(L,'left')}</div>
      <div class="rm-node" data-id="${s.id}" style="--nc:${LAYERC[s.layer]}">
        <span class="num">${s.id==='found'?'★':i}</span>
        <span class="ic">${s.ic}</span><span class="ttl">${s.title}</span>
        <span class="roles">${s.roles.map((r)=>`<span class="rb">${r}</span>`).join("")}</span>
      </div>
      <div class="rm-side right">${chips(Rr,'right')}</div>
    </div>`;
  }).join("");
  host.innerHTML = `<div class="rm"><svg class="rm-svg"></svg>${rows}</div>`;
}
function drawRoadmap(){
  const rm = document.querySelector("#roadmap .rm"); if(!rm) return;
  const svg = rm.querySelector(".rm-svg");
  svg.setAttribute("viewBox", `0 0 ${rm.clientWidth} ${rm.clientHeight}`);
  const b = rm.getBoundingClientRect();
  const R = (el)=>{const r=el.getBoundingClientRect();return{l:r.left-b.left,t:r.top-b.top,r:r.right-b.left,bo:r.bottom-b.top,cx:r.left-b.left+r.width/2,cy:r.top-b.top+r.height/2};};
  let out = "";
  const nodes=[...rm.querySelectorAll(".rm-node")];
  for(let i=0;i<nodes.length-1;i++){const a=R(nodes[i]),c=R(nodes[i+1]);out+=`<path class="ln-spine" d="M ${a.cx} ${a.bo} L ${c.cx} ${c.t}"/>`;}
  rm.querySelectorAll(".rm-child").forEach((ch)=>{
    const node=rm.querySelector(`.rm-node[data-id="${ch.dataset.stage}"]`); if(!node)return;
    const n=R(node),c=R(ch),side=ch.dataset.side;
    const x1=side==='right'?n.r:n.l, y1=n.cy, x2=side==='right'?c.l:c.r, y2=c.cy, mx=(x1+x2)/2;
    out+=`<path class="ln-branch${ch.classList.contains('opt')?' opt':''}" d="M ${x1} ${y1} C ${mx} ${y1}, ${mx} ${y2}, ${x2} ${y2}"/>`;
  });
  svg.innerHTML = out;
}

function redrawDiagrams(){ drawOverview(); drawRoadmap(); }
function renderRoleFlows(){
  const host = document.getElementById("roleflows"); if(!host) return;
  host.innerHTML = ROLEFLOWS.map((r)=>{
    const c = LAYERC[r.layer];
    return `<div class="rf" style="border-left-color:${c}">
      <div class="rf-h"><span class="ic">${r.ic}</span><span class="nm">${r.nm}</span><span class="d">${r.d}</span></div>
      <div class="rf-chain">${r.steps.map((s,i)=>
        `<span class="rf-step" data-go="${s[1]}" style="--rc:${c}"><span class="n">${i+1}</span>${s[0]}</span>`+
        (i<r.steps.length-1?'<span class="rf-arrow">›</span>':'')).join("")}</div>
    </div>`;
  }).join("");
}
renderOverview();
renderRoadmap();
renderRoleFlows();
requestAnimationFrame(redrawDiagrams);
window.addEventListener("load", redrawDiagrams);
window.addEventListener("resize", ()=>{ clearTimeout(window.__dT); window.__dT=setTimeout(redrawDiagrams,150); });

/* ===== Drawer + theo dõi tiến độ "đã nắm" (kiểu roadmap.sh — KHÔNG cuộn trang) ===== */
const MODMAP = {};
BANDS.forEach((b)=>b.modules.forEach((m)=>{ MODMAP[`d-${b.id}-${m.id}`] = {b,m}; }));

let DONE = new Set();
try { DONE = new Set(JSON.parse(localStorage.getItem("his_done") || "[]")); } catch(e){}
function saveDone(){ try{ localStorage.setItem("his_done", JSON.stringify([...DONE])); }catch(e){} }
function applyDone(){
  document.querySelectorAll("[data-go]").forEach((el)=>{
    const id = el.getAttribute("data-go");
    if(MODMAP[id]) el.classList.toggle("done", DONE.has(id));
  });
  const total = Object.keys(MODMAP).length;
  const n = [...DONE].filter((x)=>MODMAP[x]).length;
  const p = document.getElementById("prog");
  if(p) p.innerHTML = `✓ Đã nắm <span>${n}/${total}</span> phân hệ <span class="bar"><i style="width:${Math.round(n/total*100)}%"></i></span>`;
}
function toggleDone(id){
  if(DONE.has(id)) DONE.delete(id); else DONE.add(id);
  saveDone(); applyDone();
  const btn = document.getElementById("dwDone");
  if(btn){ btn.classList.toggle("on", DONE.has(id)); btn.textContent = DONE.has(id) ? "✓ Đã nắm phân hệ này" : "Đánh dấu đã nắm"; }
}

let DWID = null;
function openDrawer(id){
  const e = MODMAP[id];
  if(!e){ const t=document.getElementById(id); if(t) t.scrollIntoView({behavior:"smooth"}); return; }
  DWID = id; const {b,m} = e;
  document.getElementById("dwTitle").innerHTML =
    `<span style="font-size:22px">${m.ic}</span><span>${m.nm}</span>
     <span class="tag" style="margin-left:auto;background:${b.tag};color:#fff;font-size:11px;font-weight:700;border-radius:6px;padding:2px 8px">${b.title.split('—')[0].trim()}</span>`;
  document.getElementById("dwBody").innerHTML = `
    <button class="dw-done${DONE.has(id)?' on':''}" id="dwDone">${DONE.has(id)?'✓ Đã nắm phân hệ này':'Đánh dấu đã nắm'}</button>
    <div class="desc" style="color:var(--muted);font-size:12.5px;margin-bottom:11px">${m.desc} · <b>${m.tables.length} bảng</b></div>
    <div class="tbl-grid">${m.tables.map((x)=>`<div class="trow"><span class="vn">${x[1]}</span><span class="tn">${x[0]}</span></div>`).join("")}</div>
    ${m.rel?`<div class="rel">🔗 <b>Quan hệ chính:</b> ${m.rel}</div>`:""}`;
  const dw = document.getElementById("dw"); dw.classList.add("open"); dw.setAttribute("aria-hidden","false");
  const bk = document.getElementById("dwBack"); bk.hidden=false; requestAnimationFrame(()=>bk.classList.add("show"));
}
function closeDrawer(){
  const dw = document.getElementById("dw"); dw.classList.remove("open"); dw.setAttribute("aria-hidden","true");
  const bk = document.getElementById("dwBack"); bk.classList.remove("show"); setTimeout(()=>{bk.hidden=true;},230);
  DWID = null;
}

// chip tiến độ trên thanh điều hướng
$("#toc").insertAdjacentHTML("beforeend", '<span class="prog" id="prog"></span>');

// 1 handler duy nhất: drawer cho phân hệ (không cuộn) · band-nav vẫn cuộn mượt
document.body.addEventListener("click", (e) => {
  if(e.target.closest("#dwClose") || e.target === document.getElementById("dwBack")){ closeDrawer(); return; }
  if(e.target.closest("#dwDone")){ if(DWID) toggleDone(DWID); return; }
  const el = e.target.closest("[data-go]"); if(!el) return;
  const id = el.getAttribute("data-go");
  if(id.startsWith("band-")){ const t=document.getElementById(id); if(t) t.scrollIntoView({behavior:"smooth",block:"start"}); return; }
  openDrawer(id);
});
...
$("#q").addEventListener("input", (e) => {
 ...
 document.querySelectorAll(".mod,.detail").forEach((n)=>{
   const hit = !q || n.getAttribute("data-search").includes(q);
   n.classList.toggle("hidden", !hit);
 });
 document.querySelectorAll(".band").forEach((b)=>{
   const any = [...b.querySelectorAll(".mod")].some((m)=>!m.classList.contains("hidden"));
