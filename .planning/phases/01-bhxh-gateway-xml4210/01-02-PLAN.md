...
<task type="auto">
  ...
  <action>
    ...
    NOTE: Do NOT add controller endpoints in this task. Controller endpoint additions are handled in Task 2a below, where the controller file is listed.
  </action>
  <verify>
    cd backend/src/HIS.API && dotnet build --no-restore 2>&1 | tail -5
  </verify>
  <done>
    8 new XML table DTOs (XML6, XML8-XML11, XML13-XML15) defined with all BHXH-mandated fields. IInsuranceXmlService has 8 new generator method signatures. XmlExportConfigDto has include flags for all 15 tables. Backend builds with 0 errors.
  </done>
</task>

<task type="auto">
  <name>Task 2a: Implement core XML table generators (XML1-XML5, XML7) with real EF Core queries and add controller endpoints</name>
  <files>
    backend/src/HIS.Infrastructure/Services/InsuranceXmlService.cs
    backend/src/HIS.API/Controllers/InsuranceXmlController.cs
  </files>
  <action>
    These are the core tables with the most complex EF Core joins: the main medical record, medicine details, service details, out-of-catalog medicines, prescriptions, and referrals.
    ...
    6. **GenerateXml7DataAsync** - REWRITE to use real referral data:
    ...
    7. **Update InsuranceXmlController.cs** to expose new XML table endpoints (following existing pattern):
       - POST `/api/insurance/xml/generate/xml6` through `/xml15`
       - These are the 8 new endpoints for the DTOs defined in Task 1
  ...
  <done>
    XML1-XML5 and XML7 generators rewritten with real EF Core queries. XML1 returns full patient/claim records with Department/Room/Cost joins. XML2 returns real medicine data from ClaimDetails. XML3 returns real service data. XML4 filters non-covered medicines. XML5 extracts prescription details. XML7 uses real referral/discharge data. Controller has 8 new endpoints for XML6-XML15 tables. All money fields rounded to 2 decimals. All generators return empty lists (not null). Backend builds with 0 errors.
  </done>
</task>

<task type="auto">
  <name>Task 2b: Implement new XML table generators (XML6, XML8-XML15) with real EF Core queries</name>
  <files>
    backend/src/HIS.Infrastructure/Services/InsuranceXmlService.cs
  </files>
  <action>
    These are the new XML tables added by this plan. Most have simpler data structures or return empty lists when no data exists yet in HIS for that category. Uses the same `GetClaimsForExport` helper method created in Task 2a.

    1. **GenerateXml6DataAsync** - Blood products:
    ...
    2. **GenerateXml8DataAsync** - Patient transport:
    ...
    3. **GenerateXml9DataAsync** - Sick leave certificates:
  ...
  <verify>
    cd backend/src/HIS.API && dotnet build --no-restore 2>&1 | tail -5
    Then test new endpoints respond:
    curl -s -X POST http://localhost:5106/api/auth/login -H "Content-Type: application/json" -d '{"username":"admin","password":"Admin@123"}' | jq -r '.token' > /tmp/token.txt 2>/dev/null && curl -s -X POST http://localhost:5106/api/insurance/xml/generate/xml6 -H "Authorization: Bearer $(cat /tmp/token.txt)" -H "Content-Type: application/json" -d '{"month":2,"year":2026}' -o /dev/null -w "%{http_code}" && echo " OK"
  </verify>
  <done>
    XML6, XML8-XML15 generators implemented. XML6 queries blood bank claim details. XML11 joins patient social insurance data. XML13 queries follow-up appointments. XML14 maps referral certificates per QD 3176. XML8-XML10, XML15 return empty lists (data sources not yet available in HIS). All endpoints respond with 200. Backend builds with 0 errors.
  </done>
</task>
...
- Controller exposes 8 new endpoints for XML6-XML15
